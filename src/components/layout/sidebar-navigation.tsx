...
export function SidebarNavigation({
  ...
  return <div className="h-full flex flex-col bg-sidebar border-r border-sidebar-border">
      <div className={cn("flex h-14 items-center border-b border-sidebar-border px-4", collapsed ? "justify-center" : "justify-between")}>
        {!collapsed && <span className="text-lg font-bold text-sidebar-foreground text-center">La Rioja Cuida</span>}
        {onCollapse && <Button variant="ghost" size="icon" onClick={onCollapse} aria-label="Colapsar menú" className="text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
      ...
      <div className="flex-1 overflow-auto py-2">
        <SidebarNav links={links} collapsed={collapsed} onNavigate={handleNavigation} />
      </div>

      <UserProfile user={user} role={effectiveRole as UserRole} collapsed={collapsed} onLogout={onLogout} onNavigate={handleNavigation} />
    </div>;
}