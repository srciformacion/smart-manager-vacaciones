...
export function SidebarNavigation({
  ...
  return <div className="">
      <div className={cn("flex h-14 items-center border-b border-sidebar-border px-4", collapsed ? "justify-center" : "justify-between")}>
        {!collapsed && <span className="text-lg font-bold text-sidebar-foreground">
            La Rioja Cuida
          </span>}
        {onCollapse && <Button variant="ghost" size="icon" onClick={onCollapse} aria-label={collapsed ? "Expandir menú lateral" : "Contraer menú lateral"} className="text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
